import http from '@/utils/http.js'

/**
 * 获取当前月份的舰礼列表
 */
export function getCurrentMonthGifts() {
  return http.get('/api/captain-gifts/current')
}

/**
 * 获取指定年月的舰礼列表
 * @param {number} year - 年份
 * @param {number} month - 月份
 */
export function getGiftsByMonth(year, month) {
  return http.get('/api/captain-gifts', { params: { year, month } })
}

/**
 * 添加舰礼
 * @param {object} data - 舰礼数据
 */
export function addGift(data) {
  return http.post('/api/captain-gifts', data)
}

/**
 * 更新舰礼
 * @param {number} id - 舰礼ID
 * @param {object} data - 舰礼数据
 */
export function updateGift(id, data) {
  return http.put(`/api/captain-gifts/${id}`, data)
}

/**
 * 删除舰礼
 * @param {number} id - 舰礼ID
 */
export function deleteGift(id) {
  return http.delete(`/api/captain-gifts/${id}`)
}

/**
 * 批量添加舰礼
 * @param {object} data - 包含year, month, gifts的数据
 */
export function batchAddGifts(data) {
  return http.post('/api/captain-gifts/batch', data)
}
